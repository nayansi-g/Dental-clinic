...
function BlogCard({ blog }) {
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="flex flex-1 flex-col px-5 pb-7 pt-5 sm:px-6 sm:pb-8 sm:pt-6">
        ...
        <Link
          href={`/blog/${blog.slug}`}
          className="mt-6 inline-block text-[13px] font-medium uppercase text-[#3563e9] underline underline-offset-4 sm:text-[14px]"
        >
          Read More
        </Link>
      </div>
  ...
  return (
    <section className="bg-white px-4 py-16 sm:px-6 sm:py-20 md:px-8 lg:px-10 lg:py-24 xl:px-14">
      <div className="site-shell mx-auto w-full max-w-[1400px]">
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:mt-12 sm:gap-7 md:grid-cols-2 xl:mt-14 xl:grid-cols-3 xl:gap-8">
          {blogs.map((blog) => (
